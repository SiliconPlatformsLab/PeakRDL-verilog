...
{%- endfor %}
    ...
    clocking cb @(posedge clk);
        // We can add a skew to these signals here, but for now use this
        // clocking block to better synchronise the clock for Verilator
        // simulation. Use @(cb) instead of @(posedge clk).

        //default input #1step output #1step;
        //input  ...;   // Sample 1 simulation step before posedge
        //output valid, read, addr, wdata, wmask;   // Drive 1 simulation step after posedge
    endclocking
...
//        repeat(5) @(posedge clk);
    ...
    {%- if node is hw_readable and node.is_sw_writable %}
        ...
        {%- endif %}
            ...
            `CHECK_EQUAL({{signal(node, '', 'q')}}{{full_idx(node.parent)}}, temp[{{node|bit_range}}])

            `SW_WRITE( {{node.parent.absolute_address}}, 0 )
            `CHECK_EQUAL({{signal(node, '', 'q')}}{{full_idx(node.parent)}}, value[{{node|bit_range}}])
